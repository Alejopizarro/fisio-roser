    ...
    <div className="fixed left-1/2 top-10 transform -translate-x-1/2 bg-gradient-to-br z-1001 from-slate-50 to-slate-200 shadow-xl flex font-semibold items-center px-4 py-1 max-w-4xl mx-auto rounded-full">
      ...
      <div className="hidden sm:flex space-x-2">
        {section.map((item) => (
          <button
            key={item.name}
            className="text-gray-800 text-shadow-slate-400 min-w-max text-shadow-xl hover:bg-gray-300 h-full px-4 py-2 rounded-lg cursor-pointer transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 whitespace-nowrap w-full"
            onClick={() => handleScroll(item.scroll)}
          >
            {item.name}
          </button>
